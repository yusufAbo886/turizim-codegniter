...
// dashbord
    public function dashbord($id=0) {
        ...
        $pageData["notactive"] = $this->db->query("SELECT COUNT(*) as ctv FROM prodact WHERE status = 0")->result_object();
...
//        $pageData["prodact"] = $this->db->query("SELECT * FROM prodact WHERE 1 $theSearchADD  ")->result_object();
    ...
    public function add_edit_photo( $id=0 ){
        ...
        $pageData = array();
        $pageData['prodact'] = $this->db->query("SELECT * FROM prodact WHERE user_id = 0")->result_object();
...
//
    public function details($id =0){
       ...
       $pageData["prodact"]=$this->db->query("SELECT * FROM prodact  WHERE id = '$post_id'")->result_object();



        $this->load->view("panel/common/header", $this->headerData);
        $this->load->view("panel/detals", $pageData);
        $this->load->view("panel/common/footer", $this->footerData);
...
//            $returner["alt"]= $prodact->alt;
    ...
    public function aboutheader(){
        $pageData = array();
        $this->load->view("panel/common/header", $this->headerData);
        $this->load->view("panel/aboutheader", $pageData);
        $this->load->view("panel/common/footer", $this->footerData);

    }
    public function add_edit_aboutheader($id = 0){
        $pageData = array();
        if ($id !=0){
            $this->Checkdata->isNotNumeric($id);
            $this->Checkdata->ifEmptyOrZero($id);
            $pageData['id'] =$id;
            $returner= array();
            $aboutheader = $this->db->query("SELECT * FROM aboutheader WHERE id ='$id'")->result_object();
            if (empty($aboutheader)){
                header("Location: /panel/aboutheader/");
                exit();
            }

            $about = $aboutheader[0];
            $pageData['pageTitle'] = 'Edit the Header';
            $returner["alt"]= $about->alt;
            $pageData['thePhoto']= $about->thePhoto;
            $pageData["values"] = json_encode($returner);


        }else {
            $pageData['pageTitle'] = 'Add the Header';
            $pageData["thePhoto"] = NULL;
            $pageData['id'] = 0;
        }
        $this->load->view("panel/common/header", $this->headerData);
        $this->load->view('panel/addEditAboutheader', $pageData);
        $this->load->view("panel/common/footer", $this->footerData);
    }
...
//                    $actions .= "<div class='actionsIcon delete'><a href='javascript:;' data-href='/admin/ajax/deleteUser/{$dt->usId}' data-name='{$dt->username}' class='deleteButton'><i class='glyphicon glyphicon-trash'></i></a> </div>";
            ...
            case "getKullanci":
                ...
                foreach ($result as $dt) {
                    $actions="";
                    $actions .= "<a type='button' onclick='deleteBTN({$dt->id},\"deleteKullanci\")' class='btn btn-light-primary font-weight-bold' id='kt_sweetalert_demo_9'><i class='glyphicon glyphicon-trash text-danger'></i></a>";
            ...
            case "deleteKullanci":
                ...
                $this->db->delete("kullanci", array('id' => $id));
                echo 1;
                break;
...
//                echo 1;
            ...
            case "getAboutheader":
                ...
                $result = $this->db->query("SELECT * FROM aboutheader  ORDER BY id DESC LIMIT $iDisplayStart, $iDisplayLength")->result();
                $resultCount = $this->db->query("SELECT COUNT(1) as cnt FROM aboutheader ")->row()->cnt;
                $returnToServer = array();
                $returnToServer["sEcho"] = $sEcho;
                $returnToServer["iTotalRecords"] = $resultCount;
                $returnToServer["iTotalDisplayRecords"] = $resultCount;
                ...
                foreach ($result as $dt){
                    ...
                    $actions = "<div class='d-inline mr-4'><a class='btn btn-light-primary font-weight-bold' href='/panel/add_edit_aboutheader/{$dt->id}'><i class='flaticon2-pen text-info'></i></a></div>";
            ...
            case "addEditAboutheader":
                ...
                if ($this->form_validation->run() === FALSE) {
                    ...
                    if ($id !=0){
                        ...
                        $this->db->update("aboutheader", $processArr, array('id' => $id));
                    }else{
                        $this->db->insert("aboutheader", $processArr);
                    }
...
//                    }
                    ...
                    if ($id != 0) {
                        ...
                        if ($thePhoto && $checkPhoto) {
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user /' . $checkPhoto[0]->thePhoto);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user /thumbnail/' . $checkPhoto[0]->thePhoto);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user /medium/' . $checkPhoto[0]->thePhoto);
                        }


                        $checkPhoto1 = $this->db->query("SELECT thePhoto1 FROM prodact WHERE id = $id")->result();
                        if ($thePhoto1 && $checkPhoto1) {
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user1 /' . $checkPhoto1[0]->thePhoto1);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user1 /thumbnail/' . $checkPhoto1[0]->thePhoto1);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user1 /medium/' . $checkPhoto1[0]->thePhoto1);
                        }

                        $checkPhoto2 = $this->db->query("SELECT thePhoto2 FROM prodact WHERE id = $id")->result();
                        if ($thePhoto2 && $checkPhoto2) {
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user2 /' . $checkPhoto2[0]->thePhoto2);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user2 /thumbnail/' . $checkPhoto2[0]->thePhoto2);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user2 /medium/' . $checkPhoto2[0]->thePhoto2);
                        }
                        $checkPhoto3 = $this->db->query("SELECT thePhoto3 FROM prodact WHERE id = $id")->result();
                        if ($thePhoto3 && $checkPhoto4) {
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user3 /' . $checkPhoto3[0]->thePhoto3);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user3 /thumbnail/' . $checkPhoto3[0]->thePhoto3);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user3 /medium/' . $checkPhoto3[0]->thePhoto3);
                        }
                        $checkPhoto4 = $this->db->query("SELECT thePhoto4 FROM prodact WHERE id = $id")->result();
                        if ($thePhoto4 && $checkPhoto4) {
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user4 /' . $checkPhoto4[0]->thePhoto4);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user4 /thumbnail/' . $checkPhoto4[0]->thePhoto4);
                            unlink('https://hurenkangoedkoper.com/public1/uploads/php/files/prodact_user4 /medium/' . $checkPhoto4[0]->thePhoto4);
                        }
